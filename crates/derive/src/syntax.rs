...
fn derive_syntax_for_enum(item: ItemEnum) -> Result<proc_macro2::TokenStream> {
    ...
    let (fields, to_spans): (Vec<_>, Vec<_>) = item
        ...
        .map(|varint| {
            ...
            let parse_fields = varint
                ...
                .map(|field| {
                    ...
                    let parse = if let Some(keyword) = keyword {
                        ...
                        quote! {
                            parserc::combinators::keyword(#keyword).parse(input)
                        }
                        ...
                        quote! {
                            {
                                ...
                                if GUARD.replace(true) {
                                    Err(parserc::Kind::LeftRecursion(parserc::ControlFlow::Recovable,input.to_span_with(1)).into()) #map_err #into_fatal
                                } else {
            ...
            let to_span = quote! {
                #match_arm => {
                    let mut lhs = parserc::sourceinput::Span::None;
                    #(
                        lhs = lhs.join(&#to_spans);
                    )*
    ...
    Ok(quote! {
        impl #impl_generic parserc::syntax::Syntax<#ty_input> for #ident #type_generic #where_clause {
            #[inline]
            fn parse(input: &mut #ty_input) -> Result<Self, <#ty_input as parserc::sourceinput::Input>::Error> {
                use parserc::Parser;
                ...
                Err(parserc::Kind::Syntax(#ident_str,parserc::ControlFlow::Recovable,input.to_span_with(1)).into())#map_err
            }

            #[inline]
            fn to_span(&self) -> parserc::sourceinput::Span {
                use parserc::sourceinput::ToSpan;
                match self {
                    #(#to_spans),*
                }
            }
    ...
    if let Some(keyword) = keyword {
        Ok(quote! {
            impl #impl_generic parserc::syntax::Syntax<#ty_input> for #ident #type_generic #where_clause {
                ...
                fn parse(input: &mut #ty_input) -> Result<Self, <#ty_input as parserc::sourceinput::Input>::Error> {
                    use parserc::Parser;
                    parserc::combinators::keyword(#keyword).map(|input| Self(input)).parse(input)#map_err_global
                }
